...
class Flare_Image_Loader(data.Dataset):
	...
	def __getitem__(self, index):
		...
		if light_pos is None:
			...
			print(f"Set random light pos of {light_pos}")
		...
		return_dict = {'gt': adjust_gamma_reverse(base_img),'flare': adjust_gamma_reverse(flare_img),'lq': adjust_gamma_reverse(merge_img),'gamma':gamma}
		...
		if self.mask_type=="luminance":
			...
			if self.using_lsources:
				...
				return_dict['combo_mask']=torch.clamp(flare_mask+lsource_mask, min=0, max=1)
...
class Flare_Pair_Loader(Flare_Image_Loader):
	def __init__(self, opt):
		Flare_Image_Loader.__init__(self,opt['bg_path'],opt['transform_base'],opt['transform_flare'],opt['mask_type'])
